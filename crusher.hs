...
--    printStrBoard( revertBoards_o7m8(
--		    stateSearch_o7m8 
--    		   (initializeBoard (head history)):[]
--    		   (initializeBoards_o7m8  (tail history)) 
--		   side
--		   searchdepth
--		   n))
...
findNextMove_o7m8 history side searchdepth n findMax =
     getMinMaxBoard_o7m8 side n findMax (map f (generateNewMoves_o7m8 (head history) side))
     where f = (stateSearch_o7m8 history 
     		    	   side
			   (searchdepth-1)
			   n 
			   (not findMax)) 
...
stateSearch_o7m8 history side searchdepth n findMax initBoard
     ...
     | otherwise       	    = getMinMaxValue_o7m8 (map f (generateNewMoves_o7m8 (head history) side)) side n findMax)  	
...
getMinMaxBoard_o7m8 :: Char -> Int -> Bool -> [Board] -> Board 
getMinMaxBoard_o7m8 side n findMax genBoards 
     | ((boardEvaluator_o7m8 side n (head genBoards)) == 
        (getMinMaxValue_o7m8 genBoards side n findMax))        = (head genBoards) 
     | otherwise   = (getMinMaxBoard_o7m8 side n findMax (tail genBoards))

-- Takes in a list of generated boards and returns either
-- the highest or lowest calculated SCORE based on minmax heuristic.
-- minmax is either minimum or maximum
getMinMaxValue_o7m8 :: [Board] -> Char -> Int -> Bool -> Int 
getMinMaxValue_o7m8 genBoards side n findMax =
    getMinMax findMax (map k genBoards)    
    where k = (boardEvaluator_o7m8 side n)
